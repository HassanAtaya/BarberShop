...
export class allSRVService {
  ...
  updateLanguageByUserID(userID: any, languageID: any) {
    return this.http.post(this.url + this.languageUrl + "/updateLanguageByUserID", { userID: userID, languageID: languageID });
  }
  ...
  loadMenus() {
    if (this.checkIfPermission('ALL_PERMISSIONS')) {
      this.menus = [
        ...
        { label: 'ROLES_', route: '/roles' },
        { label: 'PERMISSIONS_', route: '/permissions' }
      ];
    }
    else {
